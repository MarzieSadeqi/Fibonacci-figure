...
def plot_fibonacci(fib_dict):
    ...
    x_values = list(fib_dict.keys())  # Indices (0 to N)
    y_values = list(fib_dict.values())  # Fibonacci numbers

    plt.figure(figsize=(8, 5))  # Create a figure
    plt.plot(x_values, y_values, marker='*', color='black')  # Plot the values
...
plot_fibonacci(fib_dict)  # Plot the Fibonacci sequence
